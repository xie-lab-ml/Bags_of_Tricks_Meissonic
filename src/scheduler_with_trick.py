...
class Scheduler(SchedulerMixin, ConfigMixin):
    ...
    def calculate_noise_factor(self, timestep):
            ratio = timestep / len(self.timesteps)
            # noise_factor = 1.0 - ratio  # 线性，可以使用其他的，类似余弦
            noise_factor = torch.cos(math.pi * timestep / (len(self.timesteps) - 1))
            return noise_factor
    ...
    def step(
        ...
        # TODO: change 1
        # ---------------------------------------------------
        noise_factor = self.calculate_noise_factor(timestep)
        ...
        # TODO: change 2
        # --------------------------------------
        low_confidence_mask = probs_.max(dim=-1).values < 0.7

        sampled_tokens = torch.where(low_confidence_mask,
                                    torch.multinomial(probs_, 1),
                                    sample.unsqueeze(-1))
        pred_original_sample = sampled_tokens.squeeze(-1)
        # ------------------------------------------
    ...
    def inv_step(
        ...
        return self.add_noise(sample, timestep, generator)
