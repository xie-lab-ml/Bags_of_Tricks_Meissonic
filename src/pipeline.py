...
from src.scheduler_inv import SchedulerInv
...
class Pipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.inv_scheduler = SchedulerInv(mask_token_id=8255)
    ...
    def forward_ours(
        ...
        # TODO: set timesteps for inversion scheduler
        self.inv_scheduler.set_timesteps(num_inference_steps, temperature, self._execution_device)

        num_warmup_steps = len(self.scheduler.timesteps) - num_inference_steps * self.scheduler.order
        
        optim_steps = len(self.scheduler.timesteps) - 1
        print("Call Zigzag Sampling!")
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, timestep in enumerate(self.scheduler.timesteps):
                ...
                if i < optim_steps:
                    ...
                    model_output = self.transformer(
                        ...
                        img_ids = img_ids,
                        txt_ids = txt_ids,
                        timestep = torch.tensor([self.scheduler.timesteps[i+1]], device=model_input.device, dtype=torch.long),
                        # guidance = 7,
                        # cross_attention_kwargs=cross_attention_kwargs,
                    )

                    # TODO: use guidance for defalut
                    uncond_logits, cond_logits = model_output.chunk(2)
                    model_output = uncond_logits + inversion_cfg * (cond_logits - uncond_logits)
    

                    # TODO: step_zigzag()
                    latents = self.inv_scheduler.step_zigzag(
                        model_output=model_output,
                        timestep=timestep,
                        sample=latents,
                        generator=generator,
                        pre_mask_tokens_sum=self.scheduler.pre_mask_tokens_sum,
                        pre_unknown_map=self.scheduler.pre_unknown_map
                    ).prev_sample
