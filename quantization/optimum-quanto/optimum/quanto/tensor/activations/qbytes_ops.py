...
@register_qbytestensor_op([torch.ops.aten.sub])
def sub(op, input, other):
    # If one of the multiplicands is a scalar, just multiply the scale
    if is_scalar(input) or input.shape == torch.Size([2, 1]):
        return ActivationQBytesTensor(other.qtype, other.size(), other.stride(), other._data, input - other._scale)
    if is_scalar(other) or other.shape == torch.Size([2, 1]):
        return ActivationQBytesTensor(input.qtype, input.size(), input.stride(), input._data, input._scale - other)
    return qfallback(op, input, other)
