...
__all__ = ["quantize", "freeze", "requantize", "quantization_map", "set_input_scale"]
...
def requantize(
    ...
    for key in model.state_dict().keys():
        if key in state_dict and model.state_dict()[key].shape != state_dict[key].shape:
...
def set_input_scale(model, i, type="set"):
    for name, m in model.named_modules():
        if isinstance(m, QModuleMixin):
            m.set_input_scale(i, type)
