...
def absmax_scale(base: torch.Tensor, qtype: qtype = qint8, axis: Optional[int] = None, if_diffusion: bool = False) -> torch.Tensor:
...
class Calibration(TorchFunctionMode):
    ...
    def calibrate_input(self, module: torch.nn.Module, input, momentum: float = 0.9):
        ...
        if isinstance(module, QModuleMixin) and module.activation_qtype is not None:
            ...
            if isinstance(input, ActivationQBytesTensor):
                ...
                if module.if_diffusion and input.ndim > 0:
                    input_scale = absmax_scale(input, module.activation_qtype, axis=0)
                else:
                    input_scale = absmax_scale(input, module.activation_qtype, axis=None)
    ...
    def calibrate_output(
        ...
        if isinstance(module, (QModuleMixin)) and module.activation_qtype is not None:
            ...
            if module.if_diffusion and output.ndim > 0:
                output_scale = absmax_scale(output, module.activation_qtype, axis=0)
            else:
