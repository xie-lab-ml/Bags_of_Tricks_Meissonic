...
class QModuleMixin(ABC):
    def __init__(
        ...
        *args,
        ...
        if_diffusion: Optional[bool] = True,
        **kwargs,
    ):
        ...
        self.if_diffusion = if_diffusion
        if not if_diffusion:
            ...
            self.register_buffer("input_scale", torch.ones((2, 1), dtype=scale_dtype, device=device))
            self.register_buffer("output_scale", torch.ones((2, 1), dtype=scale_dtype, device=device))
            for i in range(8):
                self.register_buffer(f"input_scales_{i}", torch.ones((2, 1), dtype=scale_dtype, device=device))
                self.register_buffer(f"output_scales_{i}", torch.ones((2, 1), dtype=scale_dtype, device=device))

    def set_input_scale(self, i, type="set"):
        if type == "get":
            self.input_scale = getattr(self, f"input_scales_{i}")
            self.output_scale = getattr(self, f"output_scales_{i}")
        else:
            setattr(self, f"input_scales_{i}", self.input_scale.clone())
            setattr(self, f"output_scales_{i}", self.output_scale.clone())
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        ...
        if self.if_diffusion:
            for i in range(8):
                destination[prefix + f"input_scales_{i}"] = getattr(self, f"input_scales_{i}")
                destination[prefix + f"output_scales_{i}"] = getattr(self, f"output_scales_{i}")
    ...
    def from_module(
        ...
        if qmodule.if_diffusion:
            for i in range(8):
                setattr(qmodule, f"input_scales_{i}", torch.ones_like(getattr(qmodule, f"input_scales_{i}")))
                setattr(qmodule, f"output_scales_{i}", torch.ones_like(getattr(qmodule, f"output_scales_{i}")))
        with torch.no_grad():
    ...
    def quantize_input(self, module: torch.nn.Module, input: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(input, ActivationQBytesTensor):
            ...
            input = quantize_activation(input, qtype=self.activation_qtype, scale=self.input_scale, if_diffusion=self.if_diffusion)
        return input
    ...
    def quantize_output(
        ...
        output = quantize_activation(output, qtype=self.activation_qtype, scale=self.output_scale, if_diffusion=self.if_diffusion)
        return output
